...
1. 启动时执行 `dsh web --no-open --host 127.0.0.1 --port 31080`；
2. 常驻一个托盘图标和菜单；
3. 从子进程 stdout 里认出 `dsh web: http://127.0.0.1:31080/?token=…` 这行就绪信号后，菜单项从禁用的「启动中…」变成可点的「打开 DSH」，点击后用系统默认浏览器打开这个带 token 的地址。

**用的都是 Tauri 官方 API**（`tauri::tray`/`tauri::menu` 建托盘和菜单，`tauri-plugin-opener` 开浏览器，`tauri-plugin-single-instance` 防止开两份），没有直接调用任何 macOS-only 的原生 API，理论上能跨平台编译——CI 会在 macOS / Windows / Linux 上各跑一次 `cargo check` 确认这件事，但**只在 macOS 上真正跑过、点过**。
...
- **外部杀掉壳，`dsh` 会变孤儿**：上一条说的是走「退出」菜单的情况。如果外壳是被外部信号杀掉的（比如 `kill`、系统注销/关机、Activity Monitor 强制退出），根本不会经过应用自己的退出流程，`dsh` 子进程（不只是孙进程）会整个变成孤儿继续跑——这个也实测确认过。
...
src-tauri/
  src/
    main.rs             入口
    lib.rs              Builder 装配：插件、托盘、activation policy、退出清理
    tray.rs             托盘图标 + 菜单 + 菜单事件
    dsh.rs               spawn `dsh web`、后台线程读 stdout、杀进程
