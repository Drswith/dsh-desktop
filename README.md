...
**用的都是 Tauri 官方 API**（`tauri::tray`/`tauri::menu` 建托盘和菜单，`tauri-plugin-opener` 开浏览器，`tauri-plugin-dialog` 弹退出确认框，`tauri-plugin-single-instance` 防止开两份；孤儿进程检测用了 [`sysinfo`](https://crates.io/crates/sysinfo) 做跨平台的进程查询/结束），没有直接调用任何 macOS-only 的原生 API，理论上能跨平台编译——CI 会在 macOS / Windows / Linux 上各跑一次 `cargo check` 确认这件事，但**只在 macOS 上真正跑过、点过**。
...
- **子进程清理只到一层，而且是 `SIGKILL`**：点托盘「退出」会调 `Child::kill()`——Unix 上这是 `SIGKILL`，杀得掉 `dsh` 本身，但杀不掉 `dsh` 自己再往下开的子进程（`SIGKILL` 不给它机会做清理，也没经过进程组），那些孙进程会变成孤儿继续跑。两种场景都实测验证过：直接 `sleep` 的假 `dsh`（没有子进程）能被这条路径干干净净杀掉；换成会再 `fork` 一个 HTTP server 子进程的假 `dsh` 后，退出流程杀得掉 `dsh` 自己，但那个孙进程会留下来监听着端口，得手动清。真·`dsh` 会不会自己开子进程还不确定，但这个限制是真实存在的，不是猜测。加固需要进程组（`setpgid` + 对整个组发信号），等真的需要再做。
- **孤儿清理只在下次启动时发生，而且只认「直接子进程」**：外壳被外部信号杀掉（`kill`、系统注销/关机、Activity Monitor 强制退出）时，根本不会经过应用自己的退出流程，`dsh` 子进程会整个变成孤儿——这个和上一条一样，实测确认过。这本身不算完全没处理了：`<应用数据目录>/dsh.pid` 记着上次那个子进程的 pid，**下次启动时**会去看它还在不在、名字/命令行像不像 `dsh`，像的话就杀掉再起新的——这条路径也实测过（真的模拟了一次"崩溃"：`kill -9` 杀外壳本体留下孤儿，再启动时孤儿被自动清掉了）。但两点要知道：①孤儿真正被清理是在**下一次启动**的时候，外壳如果之后一直不重开，孤儿会一直跑着；②这次清理仍然只到直接子进程这一层，跟上一条的孙进程问题是同一个没解决的坑。
...
src-tauri/
  src/
    ...
    dsh.rs               spawn `dsh web`、孤儿进程检测/清理、后台线程读 stdout、杀进程
