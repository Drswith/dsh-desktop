...
- **退出只有托盘菜单一条路**：点「退出」会先杀掉 `dsh` 子进程再退出外壳，这条路径经过实测验证有效。但如果外壳是被外部信号杀掉的（比如 `kill`、系统注销/关机、Activity Monitor 强制退出），Tauri/操作系统不会经过应用自己的退出流程，`dsh` 子进程会变成孤儿继续跑——这个也是实测确认的，不是猜测。加固它需要 Unix 信号处理 + 进程组，等真的需要再做。
