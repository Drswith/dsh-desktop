...
8. About 会显示版本、构建号、commit、构建时间、外部 `dsh --version`、`node --version`、`pnpm --version` 和系统架构；登录启动使用 Tauri 官方 autostart 插件，在 macOS 首次从 `/Applications` 启动时自动开启。

**用的都是 Tauri 官方 API/插件**（`tauri::tray`/`tauri::menu` 建托盘和菜单，`tauri::AppHandle::set_activation_policy` 控制 macOS Dock 显示，`tauri-plugin-opener` 开浏览器并定位日志文件，`tauri-plugin-dialog` 弹退出确认框、配置错误和 About，`tauri-plugin-autostart` 管理登录启动，`tauri-plugin-store` 保存用户偏好，`tauri-plugin-single-instance` 防止开两份；进程树管理在 Unix 上用独立进程组，在 Windows 上用 Job Object，孤儿记录检测用了 [`sysinfo`](https://crates.io/crates/sysinfo)），没有新增 webview，也没有直接调用 AppKit 原生 API，理论上能跨平台编译——CI 会在 macOS / Windows / Linux 上各跑一次 `cargo check` 确认这件事，但**只在 macOS 上真正跑过、点过**。
...
第一次通过菜单创建配置时，会写入带字段说明的模板；配置 JSON 无法读取或解析时会弹出“配置错误”，并暂时使用默认配置。模板里的 `runtime` 保持 `null`，只有在明确需要时才填写自定义 Node 和 entry；启动器不会安装、升级或校验 runtime。
